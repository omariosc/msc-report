...
\begin{abstract}
  ...
  \textbf{Objectives:} The primary objective is to build accurate models for tool detection and tracking in laparoscopic videos. These models are evaluated against comparable datasets, such as ART-Net, to benchmark performance.

  \textbf{Methods:} We collected an in-house dataset comprising 24 laparoscopic surgery videos, each approximately 3 minutes long in 1920x1080 resolution, totalling 1.32GB. These videos feature procedures performed by either a trainee or an expert trainer. The dataset includes 103,629 frames annotated with tooltips and ground truth data for tool position in three dimensions, including quaternions for spatial orientation and rotation values (6 degrees of freedom). Two deep learning models are implemented: one anchor-based model using the YOLOv10 architecture and a reimplementation of a state-of-the-art (SOTA) model. These models are evaluated using the standard Common Objects in Context (COCO) metric, yielding mean Average Precision (mAP) scores for detecting bounding boxes.

  \textbf{Results:} The best produced model, the anchor-based YOLOv10 model achieved an mAP50 of 99.0\% and an mAP50-95 of 80.9\% on the test set with an inference time of 25.4ms, demonstrating its effectiveness in surgical tool detection and tracking. These results highlight the potential of the models for accurate tool localization, even in resource-constrained environments.
...
\keywords{Surgical Tool Detection, Surgical Tool Tracking, Computer-assisted Laparoscopy, Surgical Training, Low and Middle-Income Countries (LMIICs), Computer Vision, Machine Learning, Deep Learning, Convolutional Neural Networks (CNNs), Medical Image Analysis, Health Informatics, Artificial Intelligence in Medicine, Surgical Education Technologies, Resource-Constrained Environments}
...
\begin{acks}
This worked was funded by the UK Engineering and Physical Sciences Research Council (EPSRC) grant \textcolor{red}{EP/S024336/1} for the University of Leeds Centre for Doctoral Training in Artificial Intelligence for Medical Diagnosis and Care. We would like to thank the supervisors Dr Dominic Jones and Dr Sharib Ali for their guidance and support throughout the project.
\end{acks}
